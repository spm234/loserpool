...
def cmd_config_set(args):
    ...
    if args.season_week1_date is not None:
        cfg.season_week1_date = args.season_week1_date
    cfg.save(conn)
...
def cmd_mark_mine(args):
    conn = db.connect(args.db)
    entry = db.get_entry_by_name(conn, args.name)
    if entry is None:
        print(f"No entry named '{args.name}' yet — it'll be created once a pick is recorded for it.", file=sys.stderr)
        sys.exit(1)
    db.set_is_mine(conn, args.name, is_mine=not args.unmark)
    print(f"{args.name}: is_mine={'0' if args.unmark else '1'}")
    conn.close()
...
def cmd_current_week(args):
    ...
    cfg = LoserPoolConfig.load(conn)
    conn.close()
    week1_str = args.week1 or cfg.season_week1_date
    if not week1_str:
        print(
            "No season_week1_date configured — set one with "
            "`config-set --season-week1-date YYYY-MM-DD` or pass --week1.",
            file=sys.stderr,
        )
        sys.exit(1)
    target = parse_iso_date(args.date) if args.date else date_cls.today()
    print(week_number_for_date(parse_iso_date(week1_str), target))


def cmd_sync_week(args):
    from .sync import sync_week

    conn = db.connect(args.db)
    also_settle = args.week - 1 if args.also_settle_previous and args.week > 1 else None
    report = sync_week(conn, args.season, args.week, api_key=args.api_key, also_settle_week=also_settle)
    print(f"Discovered {report.games_discovered} game(s), recorded {report.spreads_recorded} spread(s), "
          f"{report.results_recorded} result(s).")
...
def cmd_export_html(args):
    from .export_html import render_dashboard_html

    conn = db.connect(args.db)
    ownership = None
    if args.sheet_file:
        text = Path(args.sheet_file).read_text()
        rows = parse_pool_sheet_csv(text)
        period = args.period or WEEK_TO_PERIOD.get(args.week)
        if period:
            ownership = pick_ownership(rows, period)
    html = render_dashboard_html(conn, args.season, args.week, ownership=ownership)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument("--season-week1-date", help="ISO date, e.g. 2026-09-08 (Tuesday before the opener)")
    ...
    sp = sub.add_parser(
        "mark-mine",
        help="Flag an existing entry (e.g. one the sheet sync auto-created) as one of yours, "
        "so the dashboard shows recommendations for it",
    )
    sp.add_argument("--name", required=True)
    sp.add_argument("--unmark", action="store_true", help="Clear the flag instead of setting it")
    sp.set_defaults(func=cmd_mark_mine)
    ...
    sp = sub.add_parser(
        "current-week",
        help="Print the current NFL regular-season week number (for scripting: WEEK=$(... current-week ...))",
    )
    sp.add_argument("--week1", default=None, help="Override the configured season_week1_date")
    sp.add_argument("--date", default=None, help="Override 'today' (ISO date) — mainly for testing")
    sp.set_defaults(func=cmd_current_week)

    sp = sub.add_parser(
        "sync-week",
        help="Discover this week's games + spreads + results from The Odds API and settle picks",
    )
    sp.add_argument("--season", type=int, required=True)
    sp.add_argument("--week", type=int, required=True)
    sp.add_argument("--api-key", default=None)
    sp.add_argument(
        "--also-settle-previous", action="store_true",
        help="Also re-run settlement for week-1 (catches a late-arriving result from last run)",
    )
    sp.set_defaults(func=cmd_sync_week)

    sp = sub.add_parser("export-html", help="Render the static dashboard page")
    sp.add_argument("--season", type=int, required=True)
    sp.add_argument("--week", type=int, required=True)
