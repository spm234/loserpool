...
def set_is_mine(conn: sqlite3.Connection, display_name: str, is_mine: bool = True) -> None:
    """Flags an existing entry as 'mine' (or clears the flag) — separate
    from get_or_create_entry, since that only sets is_mine at creation
    time and won't touch it on a row that already exists (e.g. one the
    Google Sheet sync auto-created before you'd marked your own entry).
    """
    conn.execute("UPDATE lp_entry SET is_mine = ? WHERE display_name = ?", (int(is_mine), display_name))
    conn.commit()
