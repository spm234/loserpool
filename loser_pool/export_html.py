...
:root {
  ...
  --tile-bg: #f8f9fb; --banner-bg: #eaf1fd; --banner-border: #b9d3f7; --banner-text: #1a4c96;
...
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --navy: #dbe6ff; --tile-bg: #181b21; --banner-bg: #172033; --banner-border: #2c4770;
    --banner-text: #a9c6f5; --pill-chalk-bg: #2a2116; --pill-chalk-text: #e2a06a;
...
.banner {
  ...
  border-radius: 8px; padding: 10px 16px; font-size: 0.9rem; margin-bottom: 20px;
}
  ...
  <table>
    ...
    for e in my_entries:
        ...
        if len(recs) > 3:
            ...
            section += (
                f"<table><tr><th>Team</th><th>Matchup</th><th>Loses</th></tr>{more_rows}</table>"
            )
    ...
    my_html = f"<div class='card'><h2>This week's recommendations</h2>{''.join(my_sections)}</div>" if my_sections else ""
...
<div class="banner">Dashboard auto-refreshes from The Odds API and the pool's Google Sheet a few times on game days.</div>
