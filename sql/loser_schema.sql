...
CREATE TABLE IF NOT EXISTS loser_pool_config (
    ...
    -- ISO date for the start of the Week 1 game window (the Tuesday before
    -- the opener). NULL until set via config-set --season-week1-date;
    -- season_calendar.py uses it to compute "what week is it" for the
    -- automated refresh workflow. Regular season only (weeks 1-18) — see
    -- season_calendar.py's docstring for why playoff weeks aren't auto-computed.
    season_week1_date       TEXT
);
